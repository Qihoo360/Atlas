...
void t_network_backends_new() {
	...
	backends = network_backends_new(1, NULL);
	g_assert(backends);
...
void t_network_backends_check() {
	...
	/* /1* setup the test */ 
	/*  * */
	/*  * mark the backend as down *1/ */
	/* backend = network_backends_get(backends, 0); */
	/* backend->state = BACKEND_STATE_DOWN; */
	/* //g_get_current_time(&backend->state_since); */
	
	/* /1* we have to wait 4sec before it goes from DOWN to UNKNOWN *1/ */
	/* g_assert_cmpint(0, ==, network_backends_check(backends)); */
	
	/* /1* travel back in time and pretend this state is already 5 sec old *1/ */
	/* backend->state_since.tv_sec -= 5; */
	
	/* /1* we should hit the gatekeeper to reduce the number of calls to iterate the */ 
	/*  * all backends *1/ */
	/* g_assert_cmpint(0, ==, network_backends_check(backends)); */

	/* /1* open the gate *1/ */
	/* backends->backend_last_check.tv_sec -= 1; /1* pretend we checked last 1sec ago *1/ */

 	/* /1* we havn't checked any backends yet, so our backend should be set to _UNKNOWN *1/ */
	/* g_assert_cmpint(1, ==, network_backends_check(backends)); */

	/* g_assert_cmpint(BACKEND_STATE_UNKNOWN, ==, backend->state); */

	/* /1* set it to down again and check if we the caching works *1/ */
	/* backend->state = BACKEND_STATE_DOWN; */ 
	/* backend->state_since.tv_sec -= 5; */

 	/* /1* as long as we are below 1sec, no updates *1/ */
	/* g_assert_cmpint(0, ==, network_backends_check(backends)); */
