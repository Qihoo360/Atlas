...
chassis_frontend_t *chassis_frontend_new(void) {
    ...
    frontend->max_conn_for_a_backend = 1;
...
int chassis_frontend_set_chassis_options(chassis_frontend_t *frontend, chassis_options_t *opts) {
	...
	chassis_options_add(opts, "max_conn_for_a_backend", 0, 0, G_OPTION_ARG_INT, &(frontend->max_conn_for_a_backend), "max conn for a backend(default: 1)", NULL);
...
#define CHASSIS_BUILD_TAG PACKAGE_STRING
	...
	if (frontend->max_conn_for_a_backend <= 1) {
		g_critical("--max_conn_for_a_backend has to be > 1, is %d", frontend->max_conn_for_a_backend);
		GOTO_EXIT(EXIT_FAILURE);
	}
