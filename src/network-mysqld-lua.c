...
static int proxy_connection_set(lua_State *L) {
	...
	if (strleq(key, keysize, C("backend_ndx"))) {
		...
		} else if (NULL != (send_sock = network_connection_pool_lua_swap(con, backend_ndx, NULL, NULL))) {
			con->server = send_sock;
		} else if (backend_ndx == -2) {
			if (st->backend != NULL) {
				st->backend->b->connected_clients--;
				st->backend = NULL;
			}
			st->backend_ndx = -1;
...
void network_mysqld_lua_setup_global(lua_State *L , chassis *chas) {
	network_nodes_t **backends_p;
    dynamic_router_para_t **router_p;
    swap_buffer_t **forbiddenSql_p;
    ...
    //lua_pushinteger(L, chas->proxy_config->auto_restart);
    lua_pushinteger(L, chas->config->auto_restart);
    lua_setfield(L, -2, "keepalive");

    //lua_pushinteger(L, chas->proxy_config->daemon_mode);
    lua_pushinteger(L, chas->config->daemon_mode);
    lua_setfield(L, -2, "daemon");

    //lua_pushinteger(L, chas->proxy_config->event_thread_count);
    lua_pushinteger(L, chas->config->event_thread_count);
    lua_setfield(L, -2, "eventthreads");

    //lua_pushstring(L, chas->proxy_config->log_level);
    lua_pushstring(L, chas->config->log_level);
    lua_setfield(L, -2, "loglevel");

    //lua_pushstring(L, chas->proxy_config->log_path);
    lua_pushstring(L, chas->config->log_path);
    lua_setfield(L, -2, "logpath");

    //lua_pushinteger(L, chas->proxy_config->use_syslog);
    lua_pushinteger(L, chas->config->use_syslog);
    lua_setfield(L, -2, "sqllog");

    //lua_pushstring(L, chas->proxy_config->charset);
    lua_pushstring(L, chas->config->charset);
    lua_setfield(L, -2, "charset");


    lua_pushstring(L, chas->nodes->default_node);
    lua_setfield(L, -2, "default_node");
  //  lua_pushinteger(L, chas->statistics->com_select);
  //  lua_setfield(L, -2, "sele");
   lua_pop(L, 1);
    
	backends_p = lua_newuserdata(L, sizeof(network_nodes_t *));
	*backends_p = chas->nodes;
	network_backends_lua_getmetatable(L);
	lua_setmetatable(L, -2);          /* tie the metatable to the table   (sp -= 1) */
	lua_setfield(L, -2, "backends");
    
    router_p = lua_newuserdata(L, sizeof(dynamic_router_para_t *)); 
    *router_p = chas->nodes->router_para; 
	network_paras_lua_getmetatable(L);
	lua_setmetatable(L, -2);          /* tie the metatable to the table   (sp -= 1) */
	lua_setfield(L, -2, "router_paras");


    forbiddenSql_p = lua_newuserdata(L, sizeof(swap_buffer_t *)); 
    *forbiddenSql_p = chas->nodes->forbidden_sql; 
	network_forbidden_sqls_lua_getmetatable(L);
	lua_setmetatable(L, -2);          /* tie the metatable to the table   (sp -= 1) */
	lua_setfield(L, -2, "forbidden_sql");
    
	GPtrArray **raw_ips_p = lua_newuserdata(L, sizeof(GPtrArray *));
	*raw_ips_p = chas->nodes->raw_ips;
	network_clients_lua_getmetatable(L);
	lua_setmetatable(L, -2);
	lua_setfield(L, -2, "clients");

	GPtrArray **raw_pwds_p = lua_newuserdata(L, sizeof(GPtrArray *));
	*raw_pwds_p = chas->nodes->raw_pwds;
	network_pwds_lua_getmetatable(L);
    lua_setmetatable(L, -2);
    lua_setfield(L, -2, "pwds");

    statistics_t **statistics = lua_newuserdata(L, sizeof(statistics_t *)); // push addr to table 
    *statistics = chas->statistics;
    network_statistics_lua_getmetatable(L);
    lua_setmetatable(L, -2);
...
network_mysqld_register_callback_ret network_mysqld_con_lua_register_callback(network_mysqld_con *con, const char *lua_script) {
    ...
    *con_p = con;
    //TODO(dengyihao):remove by yihao
