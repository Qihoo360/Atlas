...
void test_mysqld_proto_lenenc_int(void) {
	...
	// append length < 251, write the length in first byte of the packet.data
	...
	g_assert_cmpint(0, ==, network_mysqld_proto_get_lenenc_int(&packet, &value));
	g_assert_cmpint(length, ==, value);
	g_assert_cmpint(length, ==, (guint8)packet.data->str[0]);
	...
	g_assert_cmpint(0, ==, network_mysqld_proto_get_lenenc_int(&packet, &value));
	g_assert_cmpint(length, ==, value);
	g_assert_cmpint(0xfc, ==, (guint8)packet.data->str[0]);
	...
	//  2^16 <= lenght and length < 2^24, it is stored ad 0xfd + 3-byte integer for the length 
	length = 0x10000; packet.offset = 0;
	g_string_truncate(packet.data, 0);
	g_assert(0 == network_mysqld_proto_append_lenenc_int(packet.data, length));
	g_assert_cmpint(packet.data->len, ==, 4);
	g_assert_cmpint(0, ==, network_mysqld_proto_get_lenenc_int(&packet, &value));
	g_assert_cmpint(0xfd, ==, (guint8)packet.data->str[0]);
	g_assert_cmpint(length, ==, value);
	
	// If the value is ≥ (2^24) and < (2^64) it is stored as 0xfe + 8-byte integer.
	// add by wink
	length = 0x1000000; packet.offset = 0;
	g_string_truncate(packet.data, 0);
	g_assert(0 == network_mysqld_proto_append_lenenc_int(packet.data, length));
	g_assert_cmpint(packet.data->len, ==, 9);
	g_assert_cmpint(0, ==, network_mysqld_proto_get_lenenc_int(&packet, &value));
	g_assert_cmpint(length, ==, value);
	g_assert_cmpint(0xfe, ==, (guint8)packet.data->str[0]);
 ...
 */
void test_mysqld_proto_lenenc_string(void) {
	network_packet packet;
	guint64 length = 0;
	...
	g_assert(0 == network_mysqld_proto_append_lenenc_string_len(packet.data, test_string, strlen(test_string)));
	// network_mysqld_proto_get_lenenc_string will malloc a new string, and you should free it later
	g_assert(0 == network_mysqld_proto_get_lenenc_string(&packet, &value_string, &length));
	g_assert_cmpint(strlen(test_string), ==, length);
	
	g_assert_cmpstr(value_string, ==, test_string);
	g_free(value_string);

	value_string = NULL;
	g_assert(0 != network_mysqld_proto_get_lenenc_string(&packet, &value_string, &length));
	
	g_assert(value_string == NULL);	
	g_string_free(packet.data, TRUE);
}
...
int main(int argc, char **argv) {
	...
	g_test_add_func("/core/mysql-proto-lenec-string", test_mysqld_proto_lenenc_string); // add by wink
